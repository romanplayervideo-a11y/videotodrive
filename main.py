...
def stream_to_drive(video_url, creds_json, task_id):
    process = None
    try:
        progress_data[task_id] = {"status": "Connecting...", "percent": 5}
        ...
        media = MediaIoBaseUpload(process.stdout, mimetype="video/mp4", resumable=True, chunksize=1024*1024)
        file_metadata = {"name": f"CloudBolt_{task_id}.mp4"}
        ...
        while response is None:
            ...
            if status:
                ...
                progress_data[task_id] = {"status": "Streaming...", "percent": percent}
